...
@Stateless
public class QuarterlyNotificationService  {
	
	@Inject
	private CertReceivedService cs;
	

	@Schedule(dayOfMonth="1", month="4,7,10", minute="30", hour="10", info="quarterlyTimer")
	public void yearlyMethod() {
		...
		for (CertReceived crt : mailList) {
			...
			if(crt.getCertification().getCertName().equals("CIA")) {
				...
				body = crt.getAuditor().getName()+", you have"+crt.getRemainingCpe() +" CPE to be earned till the end of year.";
				...
				body = crt.getAuditor().getName()+", you have " +crt.getRemainingCpe()+" CPE to be earned till" + crt.getNextReportingDate();
				...
				body = 	crt.getAuditor().getName()+", you have "+crt.getRemainingCpe()+" CPE to be earned till" + crt.getNextReportingDate()
						+ "You must complete "+((CehReceived)crt).getRealRemainingCpe()+" until "+((CehReceived)crt).getRealReportingDate();
